...
public class ShooterRayGun : MonoBehaviour
{
    ...
    public float shootInterval = 2f;       // Time interval between each shot
    public float projectileSpeed = 20f;    // Speed of the projectile
    public float shootingRadius = 10f;     // Radius within which the gun will shoot at the player

    private Transform player;              // Reference to the player

    private void Start()
    {
        // Find the player by tag (assuming the player is tagged as "PacMan")
        player = GameObject.FindGameObjectWithTag("PacMan").transform;
    ...
    private void Shoot()
    {
        ...
        // Check if the player is within the shooting radius
        float distanceToPlayer = Vector3.Distance(transform.position, player.position);
        if (distanceToPlayer > shootingRadius) return;
